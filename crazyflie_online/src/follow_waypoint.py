...
class MPCDemo():
    def __init__(self):
        ...
        self.rate = rospy.Rate(400) # was 250
        ...
        self.base_pos = tinyboat_trajs["circular_traj"][0]
        points = tinyboat_trajs["circular_traj"]
    ...
    def run(self):
        while not rospy.is_shutdown():
            ...
            if self.m_state == 0:
                ...
                # update the knode model if a model has been trained
                if self.controller.mode == 'knode':
                    curr_model_path = "OnlineModels/online_model{0}.pth".format(self.model_cnt)
                    if os.path.exists(curr_model_path):
                        ret = self.controller.update_model(curr_model_path)
                        # if model update succeeds, increment model count
                        if ret == 1:
                            rospy.loginfo("Updated knode controller using " + curr_model_path)
                            self.model_cnt += 1
